...
using namespace std;
struct AoA_Result {
	bool valid;             // true if a detection was found
	AoA_Estimate estimate;  // the best estimate if valid is true
};
///////////////////////////////////////////////////////////////////////////////
...
protected:
	...
	Datadeck proptable;
	//declaring Datadeck 'gaintable' that stores all gain tables
	Datadeck gaintable;

	// --- KissFFT Resources for Range Processing ---
	kiss_fft_cfg range_fft_cfg;
	kiss_fft_cpx* range_fft_in;
	kiss_fft_cpx* range_fft_out;
	int range_fft_size; // Store the size (N) used for allocation
...
public:
	...
	// virtual function to be declared in this class
	virtual double antenna_tx_gain(double az, double el);
	virtual double antenna_rx_gain(double az, double el);
	virtual double rcs_lookup(double az, double el);
	virtual double radar_range_eq(double Ptx, double Gtx, double Grx, double lambda, double R, double sigma);
	ComplexVec generate_chirp_template(int N, double Tm, double m);
	ComplexVec compute_range_profile(const ComplexVec& sweep, bool debug_dump, int N, double Tm, double m);
	ComplexVec matched_filter(const ComplexVec& sweep, const ComplexVec& template_chirp);
	virtual void adc_model(ComplexVec& sweep, int bit_depth, double voltage_range);
	virtual void synth_signal(double t_now, double m, double lambda, double Tm, double R, double Rdot, double A);
	IntMat ca_cfar(const DoubleMat& rdm, int guard_cells, int training_cells, double threshold_scale);
	IntMat log_cfar(const DoubleMat& rdm, int guard_cells, int training_cells, double threshold_dB);
	ComplexVec apply_phase_offset(const ComplexVec& sweep, double phase_offset_rad);
	AoA_Result get_best_detection(const std::vector<AoA_Estimate>& estimates, const DoubleMat& rdm_power);
	// ... other member function declarations ...
	void initialize_fft_resources(); // Helper for initialization
private:
	//DspBuffer dsp_buffer;
	DspBuffer4Ch dsp_buffer4ch;
};
///////////////////////////////////////////////////////////////////////////////
