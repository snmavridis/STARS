///////////////////////////////////////////////////////////////////////////////
//FILE: 'global_header.hpp'
//
//Global structures and classes: 'Module', 'Variable', 'File', 'Event', 'Packet'
// with inline member function definitions
//
//001206 Created by Peter H Zipfel
//011129 Adapted to MISSILE6 simulation, PZi
//081010 Modified for GENSIM simulation, PZi
//130725 Building AIM5, PZi
///////////////////////////////////////////////////////////////////////////////

#define _CRT_SECURE_NO_DEPRECATE
#ifndef global_header__HPP
#define global_header__HPP
#include <filesystem>
#include <fstream>
#include <string>		
#include "utility_header.hpp"
#include "dsp_utils.hpp"

using namespace std;


///////////////////////////////////////////////////////////////////////////////

///////////////////////////////////////////////////////////////////////////////
//Globals
//
//Generate a global path variable that captures the input directory
//Generate a global path variable that captures the repo root directory
//
//122325 Created by Savas Mavridis
///////////////////////////////////////////////////////////////////////////////

extern std::filesystem::path g_input_dir;
extern std::filesystem::path g_repo_root;


///////////////////////////////////////////////////////////////////////////////
//Structure 'Module'
//
//Provides the structure for the modules of the simulation
//
//010107 Created by Peter H Zipfel
///////////////////////////////////////////////////////////////////////////////
struct Module
{
	string name;
	string definition;
	string initialization;
	string execution;
	string termination;
};
/*
struct DspBuffer {
	ComplexMat sweeps;
	int max_sweeps = 32;
	int bins_per_sweep = 256;

	void add_sweep(const ComplexVec& sweep) {
		if (sweep.size() != bins_per_sweep) return;
		if (sweeps.size() >= max_sweeps)
			sweeps.erase(sweeps.begin());
		sweeps.push_back(sweep);
	}

	bool is_ready() const {
		return sweeps.size() == max_sweeps;
	}

	std::vector<std::vector<double>> compute_rdm() {
		std::vector<std::vector<double>> rdm(bins_per_sweep, std::vector<double>(max_sweeps, 0));
		for (int bin = 0; bin < bins_per_sweep; ++bin) {
			ComplexVec slow_time(max_sweeps);
			for (int m = 0; m < max_sweeps; ++m)
				slow_time[m] = sweeps[m][bin];
			auto fft_result = kiss_fft_wrapper(slow_time);
			for (int d = 0; d < max_sweeps; ++d)
				rdm[bin][d] = std::norm(fft_result[d]);
		}
		return rdm;
	}
};
*/
/*
struct DspBuffer4Ch {
	ComplexMat ch1, ch2, ch3, ch4;
	int max_sweeps = 32;
	int bins_per_sweep = 256;
	// --- KissFFT Resources for Doppler Processing ---
private: // Make private? Or protected if subclasses need them
	kiss_fft_cfg doppler_fft_cfg;
	kiss_fft_cpx* doppler_fft_in;
	kiss_fft_cpx* doppler_fft_out;
	bool doppler_fft_initialized;

public:
	DspBuffer4Ch();  // Constructor
	~DspBuffer4Ch(); // Destructor

	void initialize_doppler_fft(); // Initialization function
	void clear(); // Add a clear method

	void add_sweep_4ch(const ComplexVec& s1, const ComplexVec& s2, const ComplexVec& s3, const ComplexVec& s4) {
		if ((int)s1.size() != bins_per_sweep) return;

		auto trim = [this](ComplexMat& mat) {
			if ((int)mat.size() >= max_sweeps)
				mat.erase(mat.begin());
			};
		trim(ch1); trim(ch2); trim(ch3); trim(ch4);

		ch1.push_back(s1);
		ch2.push_back(s2);
		ch3.push_back(s3);
		ch4.push_back(s4);
	}

	bool is_ready() const {
		return ch1.size() == max_sweeps;
	}

	// Example: RDM from channel 1
	/*
	DoubleMat compute_rdm(const ComplexMat& channel) {
		DoubleMat rdm(bins_per_sweep, std::vector<double>(max_sweeps, 0));
		for (int bin = 0; bin < (int)bins_per_sweep; ++bin) {
			ComplexVec slow_time((int)max_sweeps);
			for (int m = 0; m < (int)max_sweeps; ++m)
				slow_time[m] = channel[m][bin];
			auto fft_result = kiss_fft_wrapper(slow_time);
			for (int d = 0; d < (int)max_sweeps; ++d)
				rdm[bin][d] = std::norm(fft_result[d]);
		}
		return rdm;
	}
	
	DoubleMat compute_rdm(const ComplexMat& channel) {
	// Ensure FFT resources are initialized (call once)
	initialize_doppler_fft(); // Safe to call multiple times due to flag

	if (!doppler_fft_initialized || channel.empty() || channel[0].empty()) {
		std::cerr << "*** Error: Doppler FFT not ready or channel empty in compute_rdm ***" << std::endl;
		return DoubleMat();
	}
	// ... (get dimensions, check consistency) ...
	int current_bins_sweep = channel[0].size(); // Get range bins from actual data
	int current_max_sweeps = channel.size(); // Get doppler bins from actual data


	DoubleMat rdm(current_bins_sweep, std::vector<double>(current_max_sweeps, 0.0));

	// Temporary vector for slow time data (size max_sweeps)
	ComplexVec slow_time_input(max_sweeps);

	for (int bin = 0; bin < current_bins_sweep; ++bin) {
		// Extract slow time data, handling potential size mismatch if buffer not full?
		// Assuming channel has exactly max_sweeps rows here for simplicity
		for (int m = 0; m < max_sweeps; ++m) {
			if (m < (int)channel.size() && bin < (int)channel[m].size()) {
				slow_time_input[m] = channel[m][bin];
			}
			else {
				slow_time_input[m] = { 0.0, 0.0 }; // Pad if needed (though is_ready should prevent this)
			}
		}

		// --- Apply Doppler Window ---
		ComplexVec windowed_slow_time(max_sweeps);
		const double PI = acos(-1.0);
		for (int m = 0; m < max_sweeps; ++m) {
			double hann_mult = 0.5 * (1.0 - cos(2.0 * PI * m / (max_sweeps - 1)));
			windowed_slow_time[m] = slow_time_input[m] * hann_mult;
		}

		// --- Perform Doppler FFT using pre-allocated resources ---
		bool success = kiss_fft_exec(this->doppler_fft_cfg, this->doppler_fft_in, this->doppler_fft_out, max_sweeps, windowed_slow_time);

		if (success) {
			// Store power results in RDM [Range Bin][Doppler Bin]
			for (int d = 0; d < max_sweeps; ++d) {
				if (bin < (int)rdm.size() && d < (int)rdm[bin].size()) { // Bounds check rdm
					rdm[bin][d] = std::norm(Complex(this->doppler_fft_out[d].r, this->doppler_fft_out[d].i));
				}
			}
		}
		else {
			std::cerr << "*** Error executing Doppler FFT for range bin " << bin << " ***" << std::endl;
			// Handle error - RDM row might be invalid
		}
	}
	return rdm;
	}
	ComplexMat get_complex_rdm(const ComplexMat& channel) {
		initialize_doppler_fft();
		if (!doppler_fft_initialized || channel.empty() || channel[0].empty()) return ComplexMat();

		int current_bins_sweep = channel[0].size();
		int current_max_sweeps = channel.size();
		ComplexMat complex_rdm(current_bins_sweep, ComplexVec(current_max_sweeps)); // Store complex results

		ComplexVec slow_time_input(max_sweeps);
		const double PI = acos(-1.0);

		for (int bin = 0; bin < current_bins_sweep; ++bin) {
			for (int m = 0; m < max_sweeps; ++m) {
				if (m < (int)channel.size() && bin < (int)channel[m].size()) {
					slow_time_input[m] = channel[m][bin];
				}
				else {
					slow_time_input[m] = { 0.0, 0.0 };
				}
			}

			ComplexVec windowed_slow_time(max_sweeps);
			for (int m = 0; m < max_sweeps; ++m) {
				double hann_mult = 0.5 * (1.0 - cos(2.0 * PI * m / (max_sweeps - 1)));
				windowed_slow_time[m] = slow_time_input[m] * hann_mult;
			}

			bool success = kiss_fft_exec(this->doppler_fft_cfg, this->doppler_fft_in, this->doppler_fft_out, max_sweeps, windowed_slow_time);

			if (success) {
				for (int d = 0; d < max_sweeps; ++d) {
					if (bin < (int)complex_rdm.size() && d < (int)complex_rdm[bin].size()) {
						complex_rdm[bin][d] = Complex(this->doppler_fft_out[d].r, this->doppler_fft_out[d].i);
					}
				}
			}
			else {
				// Handle error? Zero out row?
			}
		}
		return complex_rdm; // Return the complex RDM
	}

};
*/
///////////////////////////////////////////////////////////////////////////////
//Class 'Variable'
//Establishing module-variables as type Variable
//Provides the class for the variables used in modules
//
//001125 Created by Peter H Zipfel
//011129 Adapted to MISSILE6 simulation, PZi
//071101 Added VEC5 and MAT5, PZi
///////////////////////////////////////////////////////////////////////////////

class Variable 
{
private:
	char name[CHARN]; //label of variable
	char type[CHARN]; //type of variable 'int'; default is real
	double rval;	  //real value, double
	int ival;         //integer value, int
	Matrix VEC;		  //3x1 vector 
	Matrix MAT;		  //3x3 matrix 
	Matrix VEC5;	  //5x1 vector 
	Matrix MAT5;	  //5x5 matrix 
	char def[CHARL];  //definition and units
	char mod[CHARN];  //module where variable is calculated
	char role[CHARN]; //role that variable plays: 'data','state','diag','out','save'
	char out[CHARN];  //output for: 'scrn', 'plot', 'com'
	char error[2];	  //error code '*' = SAME LOCATION multiple, overwritten definitions
					  //           'A' = SAME NAME assigned to multiple locations 
public:
	Variable()
	{
		VEC.dimension(3,1);MAT.dimension(3,3);
		VEC5.dimension(5,1);MAT5.dimension(5,5);		
		strcpy(name,"empty");
		error[0]=' ';error[1]='\0';
		int dum=1;
	}; 
	~Variable(){};
//////////////////////////// Protopypes ///////////////////////////////////////
void init(char *na,double rv,char *de,char *mo,char *ro,char *ou);
void init(char *na,char *ty,int iv,char *de,char *mo,char *ro,char *ou);
void init(char *na,double v1,double v2,double v3,char *de,char *mo,char *ro,char *ou);
void init(char *na,double v11,double v12,double v13,double v21,double v22,double v23,
			double v31,double v32,double v33,char *de,char *mo,char *ro,char *ou);
void init(char *na,double v1,double v2,double v3,double v4,double v5,
			char *de,char *mo,char *ro,char *ou);
void init(char *na,
			double v11,double v12,double v13,double v14,double v15,
			double v21,double v22,double v23,double v24,double v25,
			double v31,double v32,double v33,double v34,double v35,
			double v41,double v42,double v43,double v44,double v45,
			double v51,double v52,double v53,double v54,double v55,
			char *de,char *mo,char *ro,char *ou);

////////////////////////// Inline Functions ///////////////////////////////////

	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'name' from module-variable array 
	//
	//001213 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char *get_name(){return name;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'type' from module-variable array 
	//
	//001213 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char *get_type(){return type;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining value (of type 'double') from module-variable array to local variable
	//Example: double thrust_com=aim[13].real();
	//
	//001128 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	double real(){return rval;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining value (of type 'int') from module-variable array to local variable
	//Example: int mprop=aim[10].integer();
	//
	//001128 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int integer(){return ival;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining vector value (of type 'Matrix') from module-variable array to local variable
	//Example: Matrix FSPV=flat3[200].vec();
	//
	//001128 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	Matrix vec(){return VEC;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining matrix value (of type 'Matrix') from module-variable array to local variable
	//Example: Matrix TGV=flat3[22].mat();
	//
	//001226 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	Matrix mat(){return MAT;}
	///////////////////////////////////////////////////////////////////////////
	//Obtaining 5x1 vector value (of type 'Matrix') from module-variable array to local variable
	//Example: Matrix X=flat3[200].vec5();
	//
	//071101 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	Matrix vec5(){return VEC5;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining 5x5 matrix value (of type 'Matrix') from module-variable array to local variable
	//Example: Matrix P=flat3[22].mat5();
	//
	//071101 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	Matrix mat5(){return MAT5;}
	
	///////////////////////////////////////////////////////////////////////////
	//Loads module-variable onto module-variable array
	//Overloaded for real and integer variables
	//Example: aim[10].gets(mprop);
	//
	//001128 Created by Peter H Zipfel
	//071101 Added vector 5x1  and matrix 5x5 member functions, PZi
	///////////////////////////////////////////////////////////////////////////
	double gets(double rv)
	{
		rval=rv;
		return rval;
	}
	
	double gets(int iv)
	{
		ival=iv;
		return ival;
	} 
	Matrix gets_vec(Matrix VE)
	{
		VEC=VE;
		return VEC;
	} 

	Matrix gets_mat(Matrix MA)
	{
		MAT=MA;
		return MAT;
	} 
	Matrix gets_vec5(Matrix VE5)
	{
		VEC5=VE5;
		return VEC5;
	} 

	Matrix gets_mat5(Matrix MA5)
	{
		MAT5=MA5;
		return MAT5;
	} 

	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'def' from module-variable array 
	//
	//001213 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char *get_def(){return def;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'mod' from module-variable array 
	//
	//001213 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char *get_mod(){return mod;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'role' from module-variable array 
	//
	//001213 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char *get_role(){return role;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'out' from module-variable array 
	//
	//001213 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char *get_out(){return out;}

	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'error' code from module-variable array 
	//
	//020909 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char *get_error(){return error;}
	
	///////////////////////////////////////////////////////////////////////////
	//Putting 'error' code into module-variable  
	//Error code must be single caracter
	//020911 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void put_error(char *error_code){strcpy(error,error_code);}
};

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
//Class 'Event'
//
//Provides the 'Event' class declaration
//
//010119 Created by Peter H Zipfel
//011129 Adapted to MISSILE6 simulation, PZi
///////////////////////////////////////////////////////////////////////////////
class Event
{
private:
	Variable *watch_variable_ptr;	//pointer to variable to be watched
	double watch_value;			//numerical value for comparison (integers will be converted later)
	char event_operator;		// < , > , =  three options of relational operators	
	int flat3_indices[NVAR];	//new variables to be read from 'flat3[]'
	double flat3_values[NVAR];	//new values to be given to variables 
	int flat3_size;				//size (number) of variables from flat3 array
	int aim_indices[NVAR];	//new variables to be read from 'aim[]'
	double aim_values[NVAR];	//new values to be given to variables
	int aim_size;				//size (number) of variables from aim array
public:
	Event(){};
	~Event(){};

	///////////////////////////////////////////////////////////////////////////
	//Setting watch variable 
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_variable(Variable *variable)
	{
		watch_variable_ptr=variable;
	}

	///////////////////////////////////////////////////////////////////////////
	//Setting watch value
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_value(double value){watch_value=value;}

	///////////////////////////////////////////////////////////////////////////
	//Setting event criterion (relational operator)
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_operator(char criterion){event_operator=criterion;}

	///////////////////////////////////////////////////////////////////////////
	//Setting index array of 'flat3' module-variable array that are used for event
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_flat3_index(int element,int index){flat3_indices[element]=index;}

	///////////////////////////////////////////////////////////////////////////
	//Setting values of new 'flat3' event variables
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_flat3_value(int element,double value){flat3_values[element]=value;}

	///////////////////////////////////////////////////////////////////////////
	//Setting size of new 'flat3' event variable array
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_flat3_size(int size){flat3_size=size;}

	///////////////////////////////////////////////////////////////////////////
	//Setting index array of 'aim' module-variable array that are used for event
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_aim_index(int element,int index){aim_indices[element]=index;}

	///////////////////////////////////////////////////////////////////////////
	//Setting values of 'aim' event variables
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_aim_value(int element,double value){aim_values[element]=value;}

	///////////////////////////////////////////////////////////////////////////
	//Setting size of new 'aim' event variable array
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_aim_size(int size){aim_size=size;}

	///////////////////////////////////////////////////////////////////////////
	//Getting watch variable
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	Variable *get_variable(){return watch_variable_ptr;}

	///////////////////////////////////////////////////////////////////////////
	//Getting value of watch variable
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	double get_value(){return watch_value;}

	///////////////////////////////////////////////////////////////////////////
	//Getting operator of event criterion
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char get_operator(){return event_operator;}

	///////////////////////////////////////////////////////////////////////////
	//Getting index array of 'flat3' module-variable array that are used for event
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int *get_flat3_indices(){return flat3_indices;}

	///////////////////////////////////////////////////////////////////////////
	//Getting values of new 'flat3' event variables
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	double *get_flat3_values(){return flat3_values;}

	///////////////////////////////////////////////////////////////////////////
	//Getting size of new 'flat3' event variable array
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int get_flat3_size(){return flat3_size;}

	///////////////////////////////////////////////////////////////////////////
	//Getting index array of 'aim' module-variable array that are used for event
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int *get_aim_indices(){return aim_indices;}

	///////////////////////////////////////////////////////////////////////////
	//Getting values of new 'aim' event variables
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	double *get_aim_values(){return aim_values;}

	///////////////////////////////////////////////////////////////////////////
	//Getting size of new 'aim' event variable array
	//
	//010123 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int get_aim_size(){return aim_size;}
};

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
//Class 'Packet'
//Provides the 'Packet' class declaration
//Packets are data clusters; one for each vehicle object, used in 'combus'
//
//010206 Created by Peter H Zipfel
//070531 New method 'set_data_variable(...)', PZi
///////////////////////////////////////////////////////////////////////////////
class Packet
{
private:
	string id;			//identification of vehicle object
	int status;			//alive=1, dead=0 . hit=-1 (targets are only stopped), 
	int ndata;			//number of module-variables in data array
	Variable *data;		//array of module-variables identified by "com" 
public:
	Packet(){};
	~Packet(){};
	///////////////////////////////////////////////////////////////////////////
	//Setting packet 'id'
	//
	//010207 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_id(string identification){id=identification;}

	///////////////////////////////////////////////////////////////////////////
	//Setting packet 'status'
	//
	//010207 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_status(int vehicle_status){status=vehicle_status;}

	///////////////////////////////////////////////////////////////////////////
	//Setting number of module variables in data
	//
	//010207 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_ndata(int vehicle_ndata){ndata=vehicle_ndata;}

	///////////////////////////////////////////////////////////////////////////
	//Setting packet 'data'
	//
	//010207 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_data(Variable *vehicle_d){data=vehicle_d;}

	///////////////////////////////////////////////////////////////////////////
	//Setting packet 'data'
	//
	//070531 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_data_variable(int slot,double value){(data+slot)->gets(value);}

	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'id' from packet 
	//
	//010207 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	string get_id(){return id;}

	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'status' from packet 
	//
	//010207 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int get_status(){return status;}

	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'ndata' from packet 
	//
	//010207 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int get_ndata(){return ndata;}

	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'data' from packet 
	//
	//010207 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	Variable *get_data(){return data;}
};

///////////////////////////////////////////////////////////////////////////////
//Class 'Document'
//stores a subset of module-variable for documentation
//
//020913 Created by Peter H Zipfel
///////////////////////////////////////////////////////////////////////////////

class Document 
{
private:
	int doc_offset;	  //marks the offset of the last entry
	char name[CHARN]; //label of variable
	char type[CHARN]; //type of variable 'int'; default is real
	char def[CHARL];  //definition and units
	char mod[CHARN];  //module where variable is calculated		
public:
	Document()
	{
		strcpy(name,"end_array");
	}
	~Document(){};

	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'doc_offset' 
	//
	//020913 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int get_doc_offset(){return doc_offset;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'name' from module-variable 
	//
	//020913 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char *get_name(){return name;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'type' from module-variable  
	//
	//020913 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char *get_type(){return type;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'def' from module-variable  
	//
	//020913 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char *get_def(){return def;}
	
	///////////////////////////////////////////////////////////////////////////
	//Obtaining 'mod' from module-variable  
	//
	//020913 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	char *get_mod(){return mod;}
	
	///////////////////////////////////////////////////////////////////////////
	//Putting 'doc_offset' 
	//
	//020913 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void put_doc_offset(int mark){doc_offset=mark;}
	
	///////////////////////////////////////////////////////////////////////////
	//Putting 'name' of module-variable 
	//
	//020913 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void put_name(char *na){strcpy(name,na);}
	
	///////////////////////////////////////////////////////////////////////////
	//Putting 'type' of module-variable  
	//
	//020913 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void put_type(char *ty){strcpy(type,ty);}
	
	///////////////////////////////////////////////////////////////////////////
	//Putting 'def' of module-variable  
	//
	//020913 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void put_def(char *de){strcpy(def,de);}
	
	///////////////////////////////////////////////////////////////////////////
	//Putting 'mod' of module-variable  
	//
	//020913 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void put_mod(char *mo){strcpy(mod,mo);}	
};

///////////////////////////////////////////////////////////////////////////////
//Class 'Table'
//Stores table data
//
//030710 Created by Peter H Zipfel
///////////////////////////////////////////////////////////////////////////////
class Table 
{
private:
	string name;	//table name 										
	int 	dim;	//table dimension (1,2, or 3)										
	int		var1_dim;	// variable 1 dimension	
	int		var2_dim;	// variable 2 dimension	
	int		var3_dim;	// variable 3 dimension	
public:
	double *var1_values;  //values of variable 1 
	double *var2_values;  //values of variable 2 
	double *var3_values;  //values of variable 3
	double *data; // table data values packaged in one array

	Table(){}
	virtual ~Table()
	{
		delete var1_values;
		delete var2_values;
		delete var3_values;
		delete data;
	}

	///////////////////////////////////////////////////////////////////////////
	//Getting dimension of table
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int get_dim(){return dim;}

	///////////////////////////////////////////////////////////////////////////
	//Getting name of table
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	string get_name(){return name;}

	///////////////////////////////////////////////////////////////////////////
	//Getting 1. independent variable dimension
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int get_var1_dim(){return var1_dim;}

	///////////////////////////////////////////////////////////////////////////
	//Getting 2. independent variable dimension
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int get_var2_dim(){return var2_dim;}

	///////////////////////////////////////////////////////////////////////////
	//Getting 3. independent variable dimension
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	int get_var3_dim(){return var3_dim;}
	
	///////////////////////////////////////////////////////////////////////////
	//Setting dimension of table
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_dim(int table_dim){dim=table_dim;}

	///////////////////////////////////////////////////////////////////////////
	//Setting name of table
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_name(string tbl_name){name=tbl_name;}

	///////////////////////////////////////////////////////////////////////////
	//Setting 1. independent variable dimension
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_var1_dim(int size){var1_dim=size;}

	///////////////////////////////////////////////////////////////////////////
	//Setting 2. independent variable dimension
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_var2_dim(int size){var2_dim=size;}

	///////////////////////////////////////////////////////////////////////////
	//Setting 3. independent variable dimension
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_var3_dim(int size){var3_dim=size;}

	///////////////////////////////////////////////////////////////////////////
	//Setting 1. independent variable values
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_var1_value(int offset,double value){
		var1_values[offset]=value;	
	}
	///////////////////////////////////////////////////////////////////////////
	//Setting 2. independent variable values
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_var2_value(int offset,double value){
		var2_values[offset]=value;	
	}
	///////////////////////////////////////////////////////////////////////////
	//Setting 3. independent variable values
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_var3_value(int offset,double value){
		var3_values[offset]=value;	
	}
	///////////////////////////////////////////////////////////////////////////
	//Setting tablular data values 
	//030710 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////
	void set_data(int offset,double value){
		data[offset]=value;	
	}
};
///////////////////////////////////////////////////////////////////////////////
//Class 'Datadeck'
//provides the 'Datadeck' class declaration for table look-ups
//
//030710 Created by Peter H Zipfel
///////////////////////////////////////////////////////////////////////////////
class Datadeck
{
private:
	string title; //title of data deck
	int capacity; //total number of tables
	int tbl_counter; //table counter
	Table **table_ptr; //table_ptr is pointer to a pointer array of type 'Table'

public:

	Datadeck(){}
	virtual ~Datadeck(){ delete [] table_ptr;}

	///////////////////////////////////////////////////////////////////////////////
	//Allocating memory  table deck title 
	//030711 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	void alloc_mem(){table_ptr=new Table *[capacity];}		

	///////////////////////////////////////////////////////////////////////////////
	//Setting table deck title 
	//030711 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	void set_title(string deck_title){title=deck_title;}

	///////////////////////////////////////////////////////////////////////////////
	//Getting table deck title 
	//030711 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	string get_title(){return title;}

	///////////////////////////////////////////////////////////////////////////////
	//Setting total number of tables 
	//030711 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	void set_capacity(int table_numbers){capacity=table_numbers;}

	///////////////////////////////////////////////////////////////////////////////
	//Getting total number of tables 
	//030711 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	int get_capacity(){return capacity;}

	///////////////////////////////////////////////////////////////////////////////
	//Setting table counter 
	//030711 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	void set_counter(int count){tbl_counter=count;}

	///////////////////////////////////////////////////////////////////////////////
	//Getting table counter 
	//030711 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	int get_counter(){return tbl_counter;}

	///////////////////////////////////////////////////////////////////////////////
	//Adding a table pointer to the table list 
	//030711 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	void add_table(Table &pt)
	{
		if(tbl_counter<capacity){
			table_ptr[tbl_counter]=&pt;
		}
	}
	///////////////////////////////////////////////////////////////////////////////
	//Overloaded operator [] returns a 'Table' pointer
	//030711 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	Table * operator[](int slot)
	{
		if(slot>=0 && slot<capacity)
			return table_ptr[slot];
		else
		{
			cout<<"*** Bad pointer value of table deck: "<<slot<<'\n';
			return 0;
		}
	}
	///////////////////////////////////////////////////////////////////////////////
	//Getting table pointer 
	//030717 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	Table * get_tbl(int slot){return table_ptr[slot];}

	///////////////////////////////////////////////////////////////////////////////
	//Single independent variable look-up
	//constant extrapolation at the upper end, slope extrapolation at the lower end
	//
	//030717 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	double look_up(string name,double value1);

	///////////////////////////////////////////////////////////////////////////////
	//Two independent variables look-up
	// Constant extrapolation at the upper end, slope extrapolation at the lower end
	//
	//030717 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	double look_up(string name,double value1,double value2);

	///////////////////////////////////////////////////////////////////////////////
	//Three independent variables look-up
	//constant extrapolation at the upper end, slope extrapolation at the lower end
	//
	//030723 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	double look_up(string name,double value1,double value2,double value3);

	///////////////////////////////////////////////////////////////////////////////
	//Table index finder
	//This is a binary search method it is O(lgN)
	//* Returns array locater of the discrete_variable just below variable
	//* Keeps max or min array locater if variable is outside those max or min  
	//
	//010628 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	int find_index(int max,double value,double *list);

	///////////////////////////////////////////////////////////////////////////////
	//Linear one-dimensional interpolation
	// Constant extrapolation beyond max values of X1
	// Slope extrapolation beyond min values of X1
	//
	//030717 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	double interpolate(int ind,int ind2,int slot,double val);

	///////////////////////////////////////////////////////////////////////////////
	//Linear, two-dimensional interpolation
	// Constant extrapolation beyond max values of X1 and X2
	// Slope extrapolation beyond min values of X1 and X2
	//
	//030718 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	double interpolate(int ind10,int ind11,int ind20,int ind21,int slot,double value1,
						double value2);																

	///////////////////////////////////////////////////////////////////////////////
	//Linear, three-dimensional interpolation
	//Constant extrapolation beyond max values of X1, X2 and X3
	//Slope extrapolation beyond min values of X1, X2 and X3
	//
	//030723 Created by Peter H Zipfel
	///////////////////////////////////////////////////////////////////////////////
	double interpolate(int ind10,int ind11,int ind20,int ind21,int ind30,int ind31,
								 int slot,double value1,double value2,double value3);																					
};

#endif
